...
void CVehiclePlayer::err_display(char* msg)
{
	LPVOID lpMsgBuf;
...
void CVehiclePlayer::err_quit(char* msg)
{
	LPVOID lpMsgBuf;
	FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM, NULL, WSAGetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), (LPTSTR)&lpMsgBuf, 0, NULL);
...
int CVehiclePlayer::recvn(SOCKET s, char* buf, int len, int flags)
{
	int received;
	char* ptr = buf;
	int left = len;
...
DWORD WINAPI RecvData(LPVOID arg)
{
	...
	while (1)
	{
		...
		if (send(*pPlayer->GetClientSocket(), (char*)&pSendData, sizeof(PlayerData), 0) == SOCKET_ERROR)
		{
			pPlayer->err_quit("send()");
		}

		for (int i = 0; i < 2; ++i)
		{
			ZeroMemory(&pRecvData, sizeof(PlayerData));
			pPlayer->recvn(*pPlayer->GetClientSocket(), (char*)&pRecvData, sizeof(PlayerData), 0);
